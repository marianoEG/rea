...
import { View, StyleSheet, Text, Alert, Dimensions, Linking } from 'react-native';
...
import { setGuestAsPresent } from '../../utils/db';
...
const QRCodeGuestScanner = () => {

    const [isProcessing, setIsProcessing] = useState<boolean>(false);
    const scannerRef = useRef<QRCodeScanner | null>(null);
    const { db } = useDbContext();
    const route = useRoute<RouteProp<RootStackParams, Screens.QRCodeGuestScanner>>();
    const SCREEN_WIDTH = Dimensions.get('window').width;
    const SCREEN_HEIGHT = Dimensions.get('window').height;

    const onQRCodeRead = (event: BarCodeReadEvent) => {
        console.log('QRCodeGuestScanner - onQRCodeRead:', event.data);
        setIsProcessing(true)
        setTimeout(() => {
            const guestDecodedStr = decrypQRCodeGuest(event.data);
            console.log('QRCodeGuestScanner - decrypQRCodeGuest:', guestDecodedStr);
            if (!guestDecodedStr) {
            ...
            try {
                ...
                if (guestDecoded?.SubeventId == route.params.subEventId)
                    saveGuestStatus(guestDecoded);
                else {
    ...
    const saveGuestStatus = async (guest: GuestDecoded): Promise<void> => {
        setGuestAsPresent(db!, guest.Id!, guest.SubeventId!)
            .then(saved => {
                if (saved)
                    showAlertAndContinue('¡Escaneo exitoso!', `El invitado '${guest.Firstname} ${guest.Lastname}' fue ingresado correctamente`);
                else
                    showAlertAndContinue('¡Advertencia!', `No se pudo establecer la presencialidad del invitado '${guest.Firstname} ${guest.Lastname}'`);
                setIsProcessing(false);
            });
    }
        ...
        <View style={styles.container}>
            <View style={styles.topBar}>
                <Text style={styles.title}>
                    Escaneá el código QR del invitado para la lista '{route.params.subEventName}'
                </Text>
            </View>
            {
                isProcessing &&
...
export default QRCodeGuestScanner;